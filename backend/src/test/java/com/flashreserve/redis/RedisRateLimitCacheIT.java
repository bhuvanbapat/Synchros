package com.flashreserve.redis;

import com.flashreserve.cache.CatalogCache;
import com.flashreserve.security.LoginAttemptLimiter;
...
@SpringBootTest(webEnvironment = SpringBootTest.WebEnvironment.NONE,
        properties = {
                "flashreserve.jobs.enabled=false",
                "flashreserve.kafka.enabled=false",
                // Tiny bucket so the boundary is reachable in one test.
                "flashreserve.rate-limit.reservations-per-minute=12",
                "flashreserve.rate-limit.burst=3",
        })
class RedisRateLimitCacheIT {
    ...
    @Autowired com.flashreserve.ratelimit.RedisRateLimiter limiter;
