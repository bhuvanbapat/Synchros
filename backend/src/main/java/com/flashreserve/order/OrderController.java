...
import com.flashreserve.payment.PaymentRelay;
import com.flashreserve.payment.PaymentSimulationDtos;
...
public class OrderController {
    ...
    @PostMapping
    public ResponseEntity<OrderResponse> create(@CurrentUser FlashUserDetails user,
    ...
    @GetMapping("/{id}")
    public OrderResponse get(@CurrentUser FlashUserDetails user, @PathVariable UUID id) {
        return OrderResponse.from(orderService.getByPublicIdForUser(id, user.getUserId()));
    }

    @GetMapping
    public List<OrderResponse> listMine(@CurrentUser FlashUserDetails user) {
