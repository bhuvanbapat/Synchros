...
import com.flashreserve.metrics.FlashMetrics;
 ...
 * flashreserve_reconciliation_findings plus flips
 * flashreserve_reconciliation_consistent to 0 — alertable from Prometheus
...
public class ReconciliationJob {
    ...
    private final FlashMetrics metrics;

    public ReconciliationJob(ReconciliationService reconciliationService,
                              FlashMetrics metrics) {
        this.reconciliationService = reconciliationService;
        this.metrics = metrics;
    }

    @Scheduled(fixedDelayString = "${flashreserve.reconciliation.interval-ms:300000}")
    public void run() {
        try {
