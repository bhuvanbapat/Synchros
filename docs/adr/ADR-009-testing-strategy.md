...
| API-level (RANDOM_PORT) | HTTP + security + idempotency end-to-end | ApiSecurityIT (IDOR, admin protection, injection), ApiIdempotencyIT (replay/conflict over real HTTP) |
...
- (+) 45 backend + 5 frontend tests, all green; the two nastiest bugs
